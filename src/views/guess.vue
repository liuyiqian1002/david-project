<template>
  <div class="guess_wrap">
    ...
    <div class="game_type">
      <ul>
        <li
          v-for="(i, index) in gameTypeList"
          :key="i"
          @click="changeGameType(index)"
          :class="{ active: gameTypeIndex == index }"
        >
          {{ i + $t("guess.min") }}
        </li>
      </ul>
    ...
    <div class="guess_panel">
      ...
      <div class="sel">
        <div
          @click="BettingTarget = 1"
          :class="['up', { active: BettingTarget == 1 }]"
        >
          <div class="pic"></div>
          <p>{{ Target(1) }}</p>
        </div>
        <div
          @click="BettingTarget = 2"
          :class="['down', { active: BettingTarget == 2 }]"
        >
          <div class="pic"></div>
          <p>{{ Target(2) }}</p>
        </div>
      ...
      <div class="txt">
        ...
        <div>
          <input
            type="text"
            v-model="Amount"
            :placeholder="$t('guess.enterAmount')"
          />
          <span>USDT</span>
        </div>
      </div>
      ...
      <div class="bot_tips">
        <div>
          <p>
            {{ $t("guess.selection") }}<span>{{ Target(BettingTarget) }}</span>
          </p>
        </div>
        <div>
          <p>
            {{ $t("guess.Payable")
              ...
              ><span class="mc">{{ Amount ? Amount : "-" }}</span
              >USDT</span
            >
          </p>
        </div>
    ...
    <div class="guess_btn">
      <button @click="showPwd" :disabled="BettingTarget == 0">
        {{ $t("guess.GuessBtn") }}
      </button>
    </div>
    <p class="tips">
      <span>{{ $t("guess.Available") }}:</span
      ><span>{{ Balance >= 0 ? Balance : "-" }} USDT</span>
    </p>
    <div class="record">
      <div class="cont">
        <van-tabs
        ...
        >
          <van-tab :title="i" v-for="i in NavBar" :key="i">
            <div class="open">
              <van-list
                v-model="loading"
                :finished="finished"
              ...
              >
                <van-empty
                  v-show="active == 0 && gameLotteryRecord.length == 0 && isRes"
                  :description="$t('base.noData')"
                  class="custom-image"
                  :image="emptyImg"
                />
                <van-empty
                  v-show="active == 1 && BettingRecord.length == 0 && isRes"
                  :description="$t('base.noData')"
                  class="custom-image"
                  :image="emptyImg"
                />
                ...
                <div class="card">
                  <p
                    class="title"
                    v-show="active == 0 && gameLotteryRecord.length != 0"
                  >
                    {{ $t("guess.LotteryRecord") }}
                  </p>
                  <p
                    class="title"
                    v-show="active == 1 && BettingRecord.length != 0"
                  >
                    {{ $t("guess.BettingRecord") }}
                  </p>
                  <!-- 开奖记录 -->
                  <ul
                    v-show="active == 0"
                    v-for="(i, index) in gameLotteryRecord"
                    :key="index"
                  >
                    <li>
                      <span>NO:{{ i.period }}</span>
                      <span
                        :class="[
                          { up: i.lotteryresult == 1 },
                          { down: i.lotteryresult == 2 },
                        ]"
                        >{{ Target(i.lotteryresult) }}</span
                      >
                    </li>
                    <li>
                      <span
                        ><label>{{ $t("guess.startprice") }}</label
                        >{{ i.startprice }}</span
                      >
                      <span
                        ><label>{{ $t("guess.endprice") }}</label
                        >{{ i.endprice }}</span
                      >
                    </li>
                  </ul>
                  ...
                  <ul
                    v-show="active == 1"
                    class="betting_record"
                    v-for="i in BettingRecord"
                    :key="i.recordid"
                  >
                    <li>
                      <span
                        ><label>NO:{{ i.period }}</label>
                        <img
                          v-show="i.deliverytype == 2"
                          src="../assets/images/common/guess/FastDelivery.png"
                          alt=""
                      /></span>
                      <span
                        ><label :class="{ end: i.status == 4 }">{{
                          status(i.status)
                        }}</label></span
                      >
                    </li>
                    <li>
                      <div>
                        <span
                          ><label>{{ $t("guess.Guess") }}</label></span
                        >
                        <span
                          :class="[
                            { up: i.bettingtarget == 1 },
                            { down: i.bettingtarget == 2 },
                          ]"
                          >{{ Target(i.bettingtarget) }}</span
                        >
                      </div>
                      <div>
                        <span
                          ><label>{{ $t("guess.Profit") }}</label></span
                        >
                        <span :class="{ mc: i.profit > 0 }">{{
                          i.profit ? i.profit : "-"
                        }}</span>
                      </div>
                    </li>
                    <li>
                      <div>
                        <span
                          ><label>{{ $t("guess.amount") }}</label></span
                        >
                        <span>{{ i.amount }} USDT</span>
                      </div>
                      <div>
                        <span
                          ><label>{{ $t("guess.Results") }}</label></span
                        >
                        <span
                          :class="[
                            { up: i.lotteryresult == 1 },
                    ...
                    <li>
                      <div>
                        <span
                          ><label>{{ $t("guess.startprice") }}</label></span
                        >
                        <span
                          ><label>{{ i.startprice }}</label></span
                        >
                      </div>
                      <div>
                        <span
                          ><label>{{ $t("guess.endprice") }}</label></span
                        >
                        <span
                          ><label>{{ i.endprice }}</label></span
                        >
                      </div>
                    </li>
                    <li>
                    ...
                    <div class="btns" v-show="i.status == 3">
                      <button @click="showFastDelivery(i.recordid)">
                        {{ $t("guess.Fast") }}
                      </button>
                    </div>
                  </ul>
    ...
    />
    <!-- 游戏说明 -->
    <van-dialog v-model="isShowRule" :title="$t('guess.ruleTitle')">
      <div class="rule" v-html="escape2Html(GamePageData.GameRule)"></div>
    </van-dialog>
    <van-dialog
      v-model="isSucces"
...
export default {
  ...
  mounted() {
    ...
    this.NavBar = [
      this.$t("guess.LotteryRecord"),
      this.$t("guess.BettingRecord"),
    ];
    ...
    this.$refs.iframe.src =
      this.$config.imgPath + `/Product/Kline?LanguageVersion=${lang}`;
  ...
  methods: {
    ...
    escape2Html(str) {
      ...
      let s = str
        ...
        : "";
      s = s
        ? s.replace(/<img/g, "<img style='max-width:100%;height:auto;'")
        : "";
    ...
    Target(val) {
      ...
      val == 0
        ? (str = "-")
        : val == 1
        ? (str = this.$t("guess.Bullish"))
        : val == 2
        ? (str = this.$t("guess.Bearish"))
        : (str = "-");
      return str;
    },
    status(val) {
      ...
      val == 0
        ? (str = "-")
        : val == 1
        ? (str = this.$t("guess.Betting"))
        : val == 2
        ? (str = this.$t("guess.drawn"))
        : val == 3
        ? (str = this.$t("guess.settled"))
        : val == 4
        ? (str = this.$t("guess.over"))
        : "";
      return str;
    },
  },
    ...
    ul {
      ...
      li {
        ...
        &:nth-of-type(2) {
          &.active {
            &.active {
              background: url(../assets/images/common/guess/type_btn2.png)
                no-repeat;
              background-size: 100% 100%;
              color: @font_3_color;
            }
        ...
        &:nth-of-type(3) {
          &.active {
            background: url(../assets/images/common/guess/type_btn3.png)
              no-repeat;
            background-size: 100% 100%;
            color: @font_3_color;
          }
    ...
    .sel {
      ...
      > div {
        ...
        &.up {
          ...
          .pic {
            margin: 0 auto;
            background: url(../assets/images/common/guess/up_un_sel.png)
              no-repeat;
          ...
          &.active {
            .pic {
              margin: 0 auto;
              background: url(../assets/images/common/guess/up_sel.png)
                no-repeat;
              background-size: contain;
            }
            p {
        ...
        &.down {
          ...
          .pic {
            margin: 0 auto;
            background: url(../assets/images/common/guess/down_un_sel.png)
              no-repeat;
            background-size: contain;
          }
          p {
          ...
          &.active {
            .pic {
              margin: 0 auto;
              background: url(../assets/images/common/guess/down_sel.png)
                no-repeat;
              background-size: contain;
            }
            p {
