<template>
  <div class="safety_wrap_main">
    <!-- <van-nav-bar
      :title="$t('safety.AccountManagement')"
      left-text
      left-arrow
      @click-left="$router.push('/safety')"
    /> -->
    <div class="safety_wrap">
      <div class="contnet">
        <ul @click="toBindAcc(i)" v-for="(i, index) in navList">
          ...
          <li>
            {{ i.val || $t("bindAccount.bind") }}
            <img src="../assets/images/common/back_right.png" alt="" />
          </li>
        </ul>
      </div>
    </div>
    <van-dialog v-model="showDialog" :title="dialogType === 'mobile' ? $t('login.navPhone') : $t('login.navEmail')" show-cancel-button>
      <bindAccount ref="bindAccount" :type="dialogType" />
    </van-dialog>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    this.navList[0].val = this.MINT_auth.Mobile
