<template>
  <div class="transaction_wrap_main">
    <!-- <van-nav-bar
      ...
      left-text
      left-arrow
      @click-right="$router.push('/withdraw_record')"
      @click-left="bank"
    >
      <template #right>
        <van-icon
    ...
    <div class="transaction_wrap">
        ...
        <ul>
          <li>
            <textarea
              ...
              class="address"
              v-model="ToAddress"
              :placeholder="$t('withdraw.ToAddressTxt')"
            />
          </li>
          <li @click="selAdd">
            <img src="../assets/images/common/add_address.png" alt="" />
          </li>
        </ul>
        ...
        <ul>
          <li>
            <input
              ...
              v-model="Amount"
              @input="enterAmount"
              :placeholder="$t('withdraw.AmountTxt')"
            />
          </li>
        </ul>
      ...
      <p class="tips" v-show="coinInfo.Coin != 'USDT'">
        {{ $t("withdraw.SetValueTips") }} :
        {{ coinInfo.SetValue | coinSetValue(1) }} {{ coinInfo.Coin }}
      </p>
      <p class="tips" v-show="coinInfo.Coin == 'USDT'">
        {{ $t("withdraw.SetValueTips") }} : {{ USDTFeeSetItem[btnIndex] }}
        {{ coinInfo.Coin }}
      </p>
      <p class="tips">
        {{ $t("withdraw.minValueTips") }} :
        {{ coinInfo.SetValue | coinSetValue(0) }} {{ coinInfo.Coin }}
      </p>
      <div class="btns">
        <button
          :disabled="coinInfo.IsWithdraw != 1"
          class="main_btn"
          @click="submitUserTakeOutCoins"
        >
          {{ $t("withdraw.btn") }}
        </button>
      </div>
...
export default {
  ...
  mounted() {
    ...
    if (this.coinInfo.Coin == "USDT") {
      this.USDTFeeSetItem = this.coinInfo.USDTFeeSetItem
        ? this.coinInfo.USDTFeeSetItem.split("#")
        : [];
    }
