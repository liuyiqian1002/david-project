  ...
  <div class="financial_wrap">
    ...
    <div class="content">
      ...
      <div class="sel_box">
        <ul>
          <li
            @click="FinancialIndex = index"
            :class="{ act: index == FinancialIndex }"
            v-for="(i, index) in baseData.PackageList"
            :key="index"
          >
            <div>
              <p>
                +{{ (i.rate * 100) | decimals(2) }}%/{{ $t("financial.day") }}
              </p>
      ...
      <div class="btns">
        <button class="main_btn" @click="showPwd">
          {{ $t("financial.btnChange") }}
        </button>
        <button class="sub_btn" @click="toManager">
          {{ $t("financial.btnManagement") }}
        </button>
      </div>
    </div>
    <van-dialog
...
export default {
  ...
  methods: {
    getData() {
      GetFinancialManagementPageData({ UserId: this.MINT_auth.UserId }).then(
        (r) => {
          if (r.data.Code == 200) {
    ...
    postTask() {
      ...
      let data = {
        ...
        ItemDetailId:
          this.baseData.PackageList[this.FinancialIndex].itemdetailid, //套餐主键
        Amount: Amount, //转入金额
      };
      // if(!this.Amount){
      //     this.$toast(this.$t('financial.AmountErr'))
