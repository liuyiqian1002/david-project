        ...
        <ul>
          <li>
            <input
              ...
              type="password"
              readonly
              :placeholder="$t('transactionPwd.password')"
              @click.stop="pwdInput('pwd')"
            />
          </li>
        </ul>
        <p>{{ $t("transactionPwd.title") }}</p>
        <ul>
          <li>
            <input
              ...
              type="password"
              readonly
              @click.stop="pwdInput('pwd1')"
              :placeholder="$t('transactionPwd.password1')"
            />
          </li>
        </ul>
      </div>
      <div class="btns">
        <button class="main_btn" @click="upUserEntity">
          {{ $t("transactionPwd.btn") }}
        </button>
      </div>
    </div>
    <van-number-keyboard
  ...
  methods: {
    getCode() {
      ...
      if (this.userInfo.VerifCodeAccount) {
        Mobile =
          this.userInfo.VerifCodeAccount == "Email"
            ? this.userInfo.Email
            : this.userInfo.AreaCode + "|" + this.userInfo.Mobile;
      } else {
        Mobile = this.userInfo.Email
          ? this.userInfo.Email
          : this.userInfo.AreaCode + "|" + this.userInfo.Mobile;
      }
      let data = {
    ...
    onDelete() {
      this[this.txtNow] = this[this.txtNow].slice(
        0,
        this[this.txtNow].length - 1
      );
    },
  },
  destroyed() {
