<template>
  <div class="blind_box_wrap">
    ...
    /> -->
    <sheader></sheader>
    <nav-header :rightTitle="$t('blindBox.title')" @rightClick="showRule"></nav-header>
    <div class="l_bg"></div>
    <div class="r_bg"></div>
    <div class="title">
      {{ $t("blindBox.goodLuck") }}
    ...
    <div class="panel">
      ...
      <div class="wrap">
        <ul>
          <li
            :class="{ active: index == BoxtIndex }"
            @click="BoxtIndex = index"
            v-for="(i, index) in BlindBoxPageData.ItemList"
            :key="i.ItemDetailId"
          >
            <div>
              <div class="pic">
        ...
        <p class="tips">
          <b>-</b>{{ replaceQuantity($t("blindBox.couponTips")) }} <b>-</b>
        </p>
        <p class="tips">
          <b>-</b>{{ replaceQuantity1($t("blindBox.couponTips1")) }} <b>-</b>
        </p>
      </div>
    </div>
    <div class="limit">
      ...
      <span
        >{{ BlindBoxPageData.TodayRemaining }} {{ $t("blindBox.Times") }}</span
      >
    </div>
    <div class="sel">
      <div class="f">
        ...
        <div @click="Quantity = 5" :class="{ active: Quantity == 5 }">
          ...
          <p>
            {{
              BoxtIndex >= 0
                ? BlindBoxPageData.ItemList[BoxtIndex].Discount * 100
                : "-"
            }}{{ $t("blindBox.discount") }}
          </p>
        </div>
    ...
    <div class="btns">
      <button @click="showBlindBoxVoucher = true">
        <img src="../../assets/images/common/guess/exchange.png" alt="" />
        {{ $t("blindBox.exchange") }}
      </button>
      <button @click="showBlindBoxRecord = true">
    ...
    <van-dialog v-model="isShowRule" :title="$t('guess.ruleTitle')">
      <div
        class="rule"
        v-html="escape2Html(BlindBoxPageData.BlindBoxRule)"
      ></div>
    </van-dialog>
    <van-dialog v-model="showBlindBoxVoucher" :title="$t('guess.ruleTitle')">
      <blind-box-voucher />
    </van-dialog>
    <van-dialog v-model="showBlindBoxRecord" :title="$t('guess.ruleTitle')">
      <blind-box-record />
    </van-dialog>
    <van-dialog
      v-model="showVoucher"
    ...
    >
      <div :class="['succes', { obtainbg: Quantity == 5 }]">
            ...
            <ul v-for="(i, index) in WinningItem" :key="index">
              <li>
                <img
                  :src="
                    i.Icon
                      ? $config.imgPath + i.Icon
                      : require('../../assets/images/common/guess/coin.png')
                  "
                  alt=""
                />
              </li>
        ...
        <div class="btn">
          <button @click="showWinning = false">
            {{ $t("blindBox.Accept") }}
          </button>
        </div>
      </div>
...
import BlindBoxVoucher from './BlindBoxVoucher.vue';
import BlindBoxRecord from './BlindBoxRecord.vue';
export default {
  name: "BlindBox",
  components: {
    BlindBoxVoucher,
  ...
  data() {
    return {
      ...
      Balance: "-",
      showVoucher: false,
      showBlindBoxVoucher: false,
      showWinning: false,
      showBlindBoxRecord: false,
      WinningItem: [
        // {
  ...
  methods: {
    ...
    openBlindBox() {
      Dialog.confirm({
        ...
        .then(() => {
          ...
          let BoxtId =
            this.BlindBoxPageData.ItemList[this.BoxtIndex].ItemDetailId;
          let data = {
    ...
    replaceQuantity(str) {
      let v =
        this.BoxtIndex >= 0
          ? this.BlindBoxPageData.ItemList[this.BoxtIndex].Quantity
          : "-";
    ...
    replaceQuantity1(str) {
      let v =
        this.BoxtIndex >= 0
          ? this.BlindBoxPageData.ItemList[this.BoxtIndex].Differences
          : "-";
      return str.replace(/{}/g, v);
    },
    escape2Html(str) {
...
//   padding: 40px;
  ...
  .panel {
    ...
    ul {
      ...
      li {
        ...
        border-radius: 10px;
        color: @font_color;
        border: 6px solid @main2_color;
        background: @bg3_color;
        &.active {
          background: @main2_color;
          background-size: 100% 100%;
          .pic {
            background: url("../../assets/images/common/guess/sel_bg.png")
              no-repeat;
            background-size: 100% 100%;
            & + p {
              color: @font_3_color;
        ...
        .pic {
          ...
          background: url("../../assets/images/common/guess/blind_box_sel.png")
            no-repeat;
          background-size: 100% 100%;
          & + p {
            // margin-top: 10px;
        ...
        > div {
          ...
          &:nth-of-type(1) {
            ...
            background: rgba(
              ...
              255,
              0.2
            ); //url('../../assets/images/common/guess/l_btn_unsel.png')no-repeat;
            background-size: 100% 100%;
            &.active {
  ...
  .succes {
    ...
    background-size: 100% 100%;
    &.obtainbg {
      background: url("../../assets/images/common/guess/success_bg1.png")
        no-repeat;
      background-size: 100% 100%;
    }
