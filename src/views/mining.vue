<template>
  <div class="mining_wrap">
    ...
    <!-- <div class="header">
      ...
      <div class="right" @click="showOrder = true">{{$t('mining.title')}}</div>
    </div> -->
    <nav-header @rightClick="showOrder = true" :rightTitle="$t('mining.title')"></nav-header>
        ...
        >
          ...
          <ul>
            ...
            <li>
              ...
              <span
                >{{ i.dailyoutput }} {{ i.coin }}/{{ $t("mining.Days") }}</span
              >
            </li>
            <li>
    ...
    <sfooter></sfooter>
    <van-dialog v-model="showOrder" width="1000px"  :title="$t('mining.title')" @confirm="showOrder = false" close-on-click-overlay show-cancel-button>
      <order />
    </van-dialog>
  </div>
</template>
...
import { GetMiningMachineList, GetEntityUser } from "@/api";
      ...
      GetEntityUser(data).then((r) => {
        if (r.data.Code == 200) {
          ...
          localStorage.setItem(
            "userInfo",
            encryptByDES(JSON.stringify(this.userInfo))
          );
        }
      });
    },
